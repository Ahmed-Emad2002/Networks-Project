    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #                 # LEAVE-ROOM-fail is sent to peer,
                #                 # if a room with this roomId and roomName does not exist
                #                 if not db.is_room_exist(message[1], message[2]):
                ...
                #                     db.leave_room(message[1], message[2], message[3])
                #                     response = "leave-room-success"
                #                     logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response)
                #                     self.tcpClientSocket.send(response.encode())
                # elif message[0] == "UPDATE_PEERS":
                #                 room = db.get_room(message[1],message[2])
                #                 peers = room["peers"]
